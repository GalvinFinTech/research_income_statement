...
def generate_professional_growth_chart_v5(
    ...
    print(f"\n--- [V5] Starting V5 (Sử dụng NhomPhanTich có sẵn) cho: {metric_to_plot} ---")
    
    # --- Bước 1 & 2: Tính toán (LOGIC GỐC CỦA V5) ---
    ...
    if metric_to_plot not in cols_to_aggregate:
        print(f"   [V5] ⚠️ Error: '{metric_to_plot}' not in cols_to_aggregate.")
        return None
    try:
        # 1. Nhóm theo 'NhomPhanTich' (đã được tạo bởi batch job)
        group_sum_nhom = df_merged.groupby(['NhomPhanTich', 'Nam', 'Quy'])[cols_to_aggregate].sum().reset_index()
        
        # 2. Tự tạo nhóm 'Toàn thị trường' (SUM tất cả)
        ...
        print("   [V5] Bước 1&2 (Tính toán) thành công.")
    except Exception as e:
        print(f"   [V5] ❌ Error in growth calculation: {e}")
        return None

    # --- Bước 3: Chuẩn bị dữ liệu (Lọc theo kỳ) ---
    print(f"   [V5] Step 3: Preparing plot data... Ending at {select_quarter} {select_year} for {lookback_periods} periods.")
    try:
        ...
        if all_periods_ts_df.empty:
            print("   [V5] ❌ LỖI: Không tìm thấy 'Toàn thị trường' trong dữ liệu.")
            return None
        ...
        end_index_list = np.where(all_periods_ts <= target_ts)[0]

        if len(end_index_list) == 0:
            print(f"   [V5] ⚠️ Không tìm thấy dữ liệu cho kỳ {select_quarter} {select_year}. Lấy {lookback_periods} kỳ cuối.")
            periods_to_plot = all_periods_ts[-lookback_periods:]
        else:
            end_index = end_index_list[-1]
            start_index = max(0, end_index - lookback_periods + 1)
            periods_to_plot = all_periods_ts[start_index : end_index + 1]
            print(f"   [V5] Lọc kỳ: Bắt đầu từ index {start_index} đến {end_index}.")

        df_plot_agg_filtered = df_plot_agg[df_plot_agg['TimePeriod'].isin(periods_to_plot)].copy()

        if df_plot_agg_filtered.empty:
            print("   [V5] ⚠️ LỖI: df_plot_agg_filtered BỊ RỖNG sau khi lọc.")
            return None
        
        # =======================================================
        # --- BẮT ĐẦU CODE DEBUG ---
        # Chúng ta sẽ in ra DataFrame TRƯỚC khi xóa NaN
        #st.info(f"DEBUG: Dữ liệu TRƯỚC khi dropna (cột cần drop: {plot_col_name})")
        #st.dataframe(df_plot_agg_filtered)
        # --- KẾT THÚC CODE DEBUG ---
        # =======================================================

        df_plot_agg_filtered.dropna(subset=[plot_col_name], inplace=True)
        
        # =======================================================
        # --- BẮT ĐẦU CODE DEBUG ---
        # Chúng ta sẽ in ra DataFrame SAU khi xóa NaN
        #st.info("DEBUG: Dữ liệu SAU khi dropna")
        #st.dataframe(df_plot_agg_filtered)
        # --- KẾT THÚC CODE DEBUG ---
        # =======================================================

        if df_plot_agg_filtered.empty:
            print(f"   [V5] ⚠️ Không có dữ liệu hợp lệ cho '{metric_to_plot}' (cột {plot_col_name}) sau khi dropping NaN.")
            return None
            
    except Exception as e:
        print(f"   [V5] ❌ Error preparing plot data: {e}")
        return None

    # --- Bước 4: Vẽ biểu đồ (Phong cách Tối giản) ---
    print(f"   [V5] Step 4: Plotting V5 (Minimalist) chart for '{metric_to_plot}'...")
    try:
        ...
        fig = px.line(
            ...
            markers=True,          
            line_shape='spline',   
            title=f'<b>Tăng trưởng Tổng {metric_to_plot} YoY theo Nhóm Phân tích</b><br><sup><i>Phương pháp: (Hiện tại - Trước) / |Trước|, giai đoạn {start_period_label}-{end_period_label}</i></sup>',
        ...
        print(f"   [V5] ✅ V5 (Minimalist+Filter) Chart plotting completed.")
        return fig
    except Exception as e:
        print(f"   [V5] ❌ Error in plotting V5 chart: {e}")
        return None